...
const registerUser = asyncHandler(async (req, res) => {
  ...
  if (!user?._id)
    return res.status(500).json({ message: "User not register. try again" });
...
const getUserById = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json({ user: user, message: "User exists" });
});

export { registerUser, login, logout, checkUserLoggedIn, getUserById };
